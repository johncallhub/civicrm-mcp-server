...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'get_contacts': {
                const { contact_id, search, contact_type, limit = 25, include_custom_fields = true } = args;
                let where = [];
                let select = ['id', 'display_name', 'first_name', 'last_name', 'organization_name', 'contact_type'];
                const joins = [
                    ['Email AS email_primary', 'LEFT', ['id', '=', 'email_primary.contact_id'], ['email_primary.is_primary', '=', true]],
                ...
                if (search)
                    where.push(['OR', [['display_name', 'LIKE', `%${search}%`], ['email_primary.email', 'LIKE', `%${search}%`]]]);
                if (contact_type)
                    where.push(['contact_type', '=', contact_type]);
                if (contact_id)
                    where.push(['id', '=', contact_id]);
            ...
            case 'create_contact': {
                ...
                const processedData = { ...standardFields };
                if (custom_fields) {
            ...
            case 'get_activities': {
                const { contact_id, activity_type, limit = 25, status, include_custom_fields = true } = args;
                let where = [];
            ...
            case 'create_activity': {
                const { custom_fields, ...activityData } = args;
            ...
            case 'get_contributions': {
                const { contact_id, limit = 25, contribution_status, financial_type, include_custom_fields = true } = args;
                let where = [];
                ...
                if (include_custom_fields) {
                    ...
                    for (const field of contributionCustomFields)
                        select.push(field.apiName);
                }
                if (contact_id)
            ...
            case 'create_contribution': {
                const { custom_fields, ...contributionData } = args;
            ...
            case 'get_events': {
                const { event_type, is_active = true, limit = 25 } = args;
                let where = [];
            ...
            case 'get_memberships': {
                const { contact_id, membership_type_id, status_id, limit = 25 } = args;
                let where = [];
                if (contact_id)
                    where.push(['contact_id', '=', contact_id]);
            ...
            case 'get_cases': {
                ...
                if (is_deleted !== undefined)
                    where.push(['is_deleted', '=', is_deleted]);
                try {
            ...
            case 'system_info': {
                try {
                    ...
                    if (systemCheck.values && systemCheck.values.length > 0) {
                        ...
                        systemCheck.values.forEach((check) => {
