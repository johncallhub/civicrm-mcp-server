...
class CiviCRMClient {
    ...
    formatResults(result, entityType) {
        if (result.error) {
    ...
    formatSingleEntity(item, entityType) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        switch (name) {
            ...
            // Address functions
            case 'get_addresses': {
                const { contact_id, address_id, location_type_id, is_primary, limit = 25 } = args;
                let where = [];
                if (contact_id)
                    where.push(['contact_id', '=', contact_id]);
                if (address_id)
                    where.push(['id', '=', address_id]);
                if (location_type_id)
                    where.push(['location_type_id', '=', location_type_id]);
                if (is_primary !== undefined)
                    where.push(['is_primary', '=', is_primary]);
            ...
            // Email functions
            case 'get_emails': {
                const { contact_id, email_id, location_type_id, is_primary, limit = 25 } = args;
                let where = [];
                if (contact_id)
                    where.push(['contact_id', '=', contact_id]);
                if (email_id)
                    where.push(['id', '=', email_id]);
                if (location_type_id)
                    where.push(['location_type_id', '=', location_type_id]);
                if (is_primary !== undefined)
                    where.push(['is_primary', '=', is_primary]);
            ...
            // Phone functions
            case 'get_phones': {
                const { contact_id, phone_id, location_type_id, phone_type_id, is_primary, limit = 25 } = args;
                let where = [];
                if (contact_id)
                    where.push(['contact_id', '=', contact_id]);
                if (phone_id)
                    where.push(['id', '=', phone_id]);
                if (location_type_id)
                    where.push(['location_type_id', '=', location_type_id]);
                if (phone_type_id)
                    where.push(['phone_type_id', '=', phone_type_id]);
                if (is_primary !== undefined)
                    where.push(['is_primary', '=', is_primary]);
            ...
            case 'update_phone': {
                const { phone_id, ...updateData } = args;
            ...
            // Website functions
            case 'get_websites': {
                const { contact_id, website_id, website_type_id, limit = 25 } = args;
                let where = [];
                if (contact_id)
                    where.push(['contact_id', '=', contact_id]);
                if (website_id)
                    where.push(['id', '=', website_id]);
                if (website_type_id)
                    where.push(['website_type_id', '=', website_type_id]);
            ...
            case 'update_website': {
                const { website_id, ...updateData } = args;
            ...
            case 'get_contact_types': {
                const { contact_type_id, name, parent_id, is_active = true, limit = 25 } = args;
                let where = [];
                if (contact_type_id)
                    where.push(['id', '=', contact_type_id]);
                if (name)
            ...
            case 'get_group_contacts': {
                const { group_id, contact_id, status, limit = 50 } = args;
                let where = [];
                if (group_id)
            ...
            case 'remove_contact_from_group': {
                const { group_id, contact_id } = args;
            ...
            case 'get_relationship_types': {
                const { relationship_type_id, name_a_b, name_b_a, is_active = true, limit = 25 } = args;
                let where = [];
                if (relationship_type_id)
                    where.push(['id', '=', relationship_type_id]);
                if (name_a_b)
                    where.push(['name_a_b', '=', name_a_b]);
                if (name_b_a)
                    where.push(['name_b_a', '=', name_b_a]);
                if (is_active !== undefined)
                    where.push(['is_active', '=', is_active]);
            ...
            case 'get_entity_tags': {
                const { entity_table, entity_id, tag_id, limit = 50 } = args;
                let where = [];
                if (entity_table)
                    where.push(['entity_table', '=', entity_table]);
                if (entity_id)
                    where.push(['entity_id', '=', entity_id]);
                if (tag_id)
                    where.push(['tag_id', '=', tag_id]);
            ...
            case 'remove_entity_tag': {
                const { entity_table, entity_id, tag_id } = args;
            ...
            case 'get_membership_types': {
                const { membership_type_id, name, member_of_contact_id, is_active = true, limit = 25 } = args;
                let where = [];
                if (membership_type_id)
                    where.push(['id', '=', membership_type_id]);
                if (name)
                    where.push(['name', '=', name]);
                if (member_of_contact_id)
                    where.push(['member_of_contact_id', '=', member_of_contact_id]);
                if (is_active !== undefined)
                    where.push(['is_active', '=', is_active]);
            ...
            case 'get_membership_statuses': {
                const { membership_status_id, name, is_active = true, limit = 25 } = args;
                let where = [];
                if (membership_status_id)
                    where.push(['id', '=', membership_status_id]);
                if (name)
                    where.push(['name', '=', name]);
                if (is_active !== undefined)
                    where.push(['is_active', '=', is_active]);
...
//# sourceMappingURL=index_enhanced.js.map